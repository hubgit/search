...
interface SearchConfig {
  ...
  // Whether to treat string searches literally by default when the search
  /// panel is activated (defaults to false).
  literal?: boolean
  ...
  /// Whether to enable "search within selection" by default when the search
  /// panel is activated (defaults to false).
...
const searchConfigFacet: Facet<SearchConfig, Required<SearchConfig>> = Facet.define({
  combine(configs) {
    return {
      ...
      withinSelection: configs.reduce((val, conf) => val ?? conf.withinSelection, undefined as boolean | undefined) || false,
...
function defaultQuery(state: EditorState, fallback?: SearchQuery) {
  ...
  return new SearchQuery({
    ...
    withinSelection: fallback?.withinSelection ?? config.wholeWord,
  })
}
